 ...
 *
 *      starts in current directory ans recursively
 *      climbs upp to root of filesystem, prints top part
 *      then prints current part
 *
 *      uses readdir() to get info about each thing
 *
 *      bug: prints an empty string if runfrom "/"
 */
#include        <stdio.h>
#include        <sys/types.h>
#include        <sys/stat.h>
...
/*
 *      prints path leading down to an objact with this inode
 *      kindof recursive
 */
        ino_t   my_inode;
        ...
        if(get_inode("..")!=this_inode){
                ...
                printpathto(my_inode);                          /* now print    */
